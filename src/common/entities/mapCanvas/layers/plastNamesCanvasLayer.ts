...
export class PlastNamesCanvasLayer extends CommonCanvas implements MapLayer {
    public data: PlastNameModel[];
    public visible: boolean;
    public canvasSize: CanvasSize;

    private names: {
        [index: string]: PlastNameModel[];
    };

    public constructor(data: PlastNameModel[], visible: boolean, canvasSize: CanvasSize) {
        super(canvasSize);
    ...
    public initLayer = (model: InitMapModel): void => {
        ...
        const data = filter(it => !isNullOrEmpty(it.plastNames) && it.plastNames.length > 1, this.data) ?? [];
        ...
        this.names = groupByProp(
            ...
            R.map(
                it => ({
                    ...
                    x: this.cx(it.x),
                    y: this.cy(it.y),
                    plastNames: reverse(it.plastNames)
                }),
                data
            )
        );
    ...
    public render = (model: MapModel): void => {
        ...
        mapObjIndexed(group => {
            ...
            const plastNames = group[0].plastNames;
