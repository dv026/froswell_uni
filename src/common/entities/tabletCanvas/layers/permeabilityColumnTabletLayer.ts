...
export class PermeabilityColumnTabletLayer extends BaseColumnTabletLayer implements TabletLayer {
    ...
    public equals(other: PermeabilityColumnTabletLayer): boolean {
        if (isNil(other)) {
            return false;
        }
    ...
    public clone(): PermeabilityColumnTabletLayer {
        return new PermeabilityColumnTabletLayer(this.column, this.canvasSize);
    }
    ...
    public initLayer = (model?: InitTabletModel): void => {
        ...
        forEach(it => {
            ...
            this.items.push({
                color: opacity(colors.colors.darkblue, 0.5),
                rect: [
                    model.cx(scope[0]),
                    model.cy(topY),
                    model.cx(scope[0] + this.widthRangeScale(this.column, model.columns, it.permeability)),
                    model.cy(bottomY)
                ],
                tooltip: head(tooltipLabelNumberParameter(column, it.permeability))
            });
        }, model.model.data);
