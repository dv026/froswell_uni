...
import { find, forEach, head, includes, isNil } from 'ramda';
...
export class PorosityColumnTabletLayer extends BaseColumnTabletLayer implements TabletLayer {
    ...
    public equals(other: PorosityColumnTabletLayer): boolean {
        if (isNil(other)) {
            return false;
        }
    ...
    public clone(): PorosityColumnTabletLayer {
        return new PorosityColumnTabletLayer(this.column, this.canvasSize);
    }
    ...
    public initLayer = (model?: InitTabletModel): void => {
        ...
        forEach(it => {
            ...
            this.items.push({
                color: opacity(colors.colors.green, 0.5),
                rect: [
                    model.cx(scope[0]),
                    model.cy(topY),
                    model.cx(scope[0] + this.widthRangeScale(this.column, model.columns, it.porosity)),
                    model.cy(bottomY)
                ],
                tooltip: head(tooltipLabelNumberParameter(column, it.porosity))
            });
        }, model.model.data);
