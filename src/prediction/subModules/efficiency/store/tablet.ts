...
export const tabletData = selector<TabletDataModel>({
    key: 'predictionEfficiency__tabletData',
    get: async ({ get }) => {
        ...
        const { data } = await getTabletData(well, currentWells, EvaluationTypeEnum.Insim, gtmType);
...
export const defaultTabletSettings = selector<TabletSettingsModel>({
    key: 'predictionEfficiency__defaultTabletSettings',
    get: async ({ get }) => {
...
export const tabletSettingsState = atom<TabletSettingsModel>({
    key: 'predictionEfficiency__tabletSettingsState',
    default: defaultTabletSettings
});

export const tabletCanvasSize = selector<CanvasSize>({
    key: 'predictionEfficiency__tabletCanvasSize',
    get: async ({ get }) => {
        const model = get(tabletData);
