...
export const gridMapVariationSettings = selector<GridMapSettings>({
    key: 'predictionResults__gridMapVariationSettings',
    get: async ({ get }) => {
...
export const gridMapSettings = selector<GridMapSettings>({
    key: 'predictionMap__gridMapSettings',
    get: async ({ get }) => {
        ...
        const response = await getMapGrid(gridMap, [well], plastId, historyDate, isolineSettings);
