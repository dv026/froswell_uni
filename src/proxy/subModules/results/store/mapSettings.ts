...
import { MapSettingModel } from '../../../../prediction/entities/mapSettingModel';
import { getMapByParams } from '../../../../prediction/subModules/results/gateways/gateway';
...
const mapSettingsRefresherState = atom({
    key: 'proxyResults__mapSettingsRefresherState',
    default: 0
});

export const mapSettingsRefresher = selector({
    key: 'proxyResults__mapSettingsRefresher',
    get: async ({ get }) => {
...
const mapSettingsLoad = selector<MapSettingModel>({
    key: 'proxyResults__mapSettingsLoad',
    get: async ({ get }) => {
...
export const availableGridsSelector = selector<GridMapEnum[]>({
    key: 'proxyResults__availableGridsSelector',
    get: async ({ get }) => {
...
export const krigingPeriodSelector = selector<Date[]>({
    key: 'proxyResults__krigingPeriodSelector',
    get: async ({ get }) => {
        const mapSettings = get(mapSettingsState);
