...
import { firstObjectFromList, firstObjectWithScenario, firstWellFromList } from './well';
...
export function useWellMutations() {
    ...
    const set = useRecoilCallback(({ snapshot, set, reset }) => async (well: WellBrief) => {
        ...
        const step = await snapshot.getPromise(currentStep);
        ...
        if (!well) {
            if (subModule === SubModuleType.Calculation) {
                ...
                currentWell = firstObjectFromList(list);
            ...
            if (subModule === SubModuleType.Calculation) {
                ...
                if (step === DirectedStageEnum.Calculation && router.containsScenario(well, list)) {
                    currentWell = well;
                } else if (router.containsProductionObject(well, list)) {
                    // TIP: необходимо получить объект с сценарием, чтобы отобразить сценарий и ошибки по нему в улучшении
                    currentWell = firstObjectWithScenario(list, well.prodObjId) || well;
                ...
                if (router.containsWellWithScenario(well, list)) {
                    currentWell = well;
                } else if (router.containsScenario(well, list)) {
                    currentWell = well;
                } else {
                ...
                if (router.containsWellWithSubScenario(well, list) || router.containsWellWithScenario(well, list)) {
